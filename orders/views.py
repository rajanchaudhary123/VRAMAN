...
def place_order(request):
    ...
    vendors_ids = []
    for i in cart_items:
        if i.packageitem.vendor.id not in vendors_ids:
            vendors_ids.append(i.packageitem.vendor.id)
    ...
    k = {}
    for i in cart_items:
        packageitem = PackageItem.objects.get(pk=i.packageitem.id, vendor_id__in=vendors_ids)
        
        v_id = packageitem.vendor.id
        if v_id in k:
            subtotal = k[v_id]
            subtotal += (packageitem.price * i.quantity)
            k[v_id] = subtotal
        else:
            subtotal = (packageitem.price * i.quantity)
            k[v_id] = subtotal
        ...
        total_data.update({packageitem.vendor.id: {str(subtotal): str(tax_dict)}})
